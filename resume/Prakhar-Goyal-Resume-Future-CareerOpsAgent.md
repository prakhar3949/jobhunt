# Prakhar Goyal
**AI Engineer — Applied Agentic Tooling**
...
## Summary
Data scientist and AI engineer with **9+ years** building and shipping ML systems end-to-end — from modeling to production MLOps. Drove **$150M+ in incremental revenue** at FedEx through ML-powered retention and lifetime-value products. Now focused on **applied agentic tooling that solves real personal/business workflows**: a multi-agent career-ops system that scores and tailors resumes against job descriptions with PII-safe handling and persistent memory, alongside a multi-tool quantitative-finance agent and a full AgentOps layer. MS in Business Analytics (GPA 3.96).
...
**Agent Orchestration & Memory:** Multi-agent pipelines (parse → score → assemble), persistent agent memory (episodic/semantic, BaseStore backend), LangChain `deepagents`, human-in-the-loop (HITL) approval gates
**Guardrails & Safety:** PII detection/redaction before LLM calls, input/output filtering, Pydantic structured-output validation, prompt-injection defense
...
- Grounded a PDF/document RAG stage over past job descriptions and interview notes, enabling "have I seen a role like this before?" queries.
